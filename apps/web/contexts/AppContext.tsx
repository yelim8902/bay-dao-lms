...
// 공통 패키지에서 타입 import
export type { Assignment, Cohort, Certificate, StudentEnrollment, DAOStats };
...
type AppAction =
  ...
  | {
      ...
      payload: { studentAddress: string; cohortId: string };
    };
...
const initialState: AppState = {
  assignments: [
    {
      ...
      title: "이더리움 기초 공부하기",
      description:
        "이더리움의 기본 개념, 스마트 컨트랙트, 가스비 등에 대해 학습하고 정리하기",
      dueDate: "2024-02-15",
      status: "pending",
      maxScore: 100,
      weight: 15,
      isRequired: true,
      cohortId: "Bay Research Track 2024", // 실제 기수 ID로 수정
    ...
    },
    {
      ...
      title: "DAO 거버넌스 이해하기",
      description:
        "DAO의 개념, 거버넌스 토큰, 제안 및 투표 시스템에 대해 학습하기",
      dueDate: "2024-02-22",
      status: "pending",
      maxScore: 100,
      weight: 20,
  ...
  cohorts: [
    {
      id: "Bay Research Track 2024", // 실제 기수 ID로 수정
    ...
    {
      id: "Bay Development Track 2024", // 실제 기수 ID로 수정
  ...
  enrollments: [
    ...
    // 현재 사용자용 테스트 등록 (실제 주소로 교체 필요)
    {
      studentAddress: "0x3c28d5F5Bb3CAB353663132Fe052e5A386dD5906", // 실제 사용자 주소
...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case "SUBMIT_ASSIGNMENT":
      const updatedState = {
        ...
        enrollments: state.enrollments.map((enrollment) => {
          if (
            ...
            const updatedCompletedAssignments = [
              ...enrollment.completedAssignments,
              action.payload.assignmentId,
            ];
            const cohortAssignments = state.assignments.filter(
              (a) => a.cohortId === action.payload.cohortId
            );
            const canRefund =
              updatedCompletedAssignments.length === cohortAssignments.length;
      ...
      // 모든 과제가 완료되면 인증서 생성
      const enrollment = updatedState.enrollments.find(
        (e) =>
      ...
      if (enrollment) {
        ...
        const allAssignmentsCompleted =
          enrollment.completedAssignments.length === cohortAssignments.length;

        if (allAssignmentsCompleted) {
          const cohort = state.cohorts.find(
            (c) => c.id === action.payload.cohortId
          );
